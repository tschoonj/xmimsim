...
int main(int argc, char *argv[]) {
	...
	g_assert_true(xmi_input_equals(input, input_copy));
	...
	GError *error = NULL;
	g_assert_null(input = xmi_input_read_from_xml_file("non-existent-file.xmsi", &error));
	...
	g_assert_null(input = xmi_input_read_from_xml_file(TEST_XMSI_COPY, &error));
	g_assert_true(g_error_matches(error, XMI_MSIM_ERROR, XMI_MSIM_ERROR_XML));
	fprintf(stdout, "message: %s\n", error->message);
	g_clear_error(&error);
