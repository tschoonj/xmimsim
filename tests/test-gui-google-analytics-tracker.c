...
static void test_send_from_local_tracker_fixed_uuid(SetupData *data, gconstpointer user_data) {
	XmiMsimGuiGoogleAnalyticsTracker *tracker = xmi_msim_gui_google_analytics_tracker_new(TEST_UUID);

	g_assert_nonnull(tracker);

	g_signal_connect(G_OBJECT(tracker), "after-event", G_CALLBACK(after_event_cb), data);

	g_assert_true(xmi_msim_gui_google_analytics_tracker_send_event(tracker, "MAKE-CHECK-TEST", "LOCAL TRACKER FIXED UUID", NULL, NULL));
...
static void test_send_from_global_tracker_fixed_uuid(SetupData *data, gconstpointer user_data) {
	xmi_msim_gui_google_analytics_tracker_create_global(TEST_UUID);
	...
	g_assert_true(xmi_msim_gui_google_analytics_tracker_send_event(tracker, "MAKE-CHECK-TEST", "GLOBAL TRACKER FIXED UUID", NULL, NULL));
